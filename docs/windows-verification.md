...
Start the visual test app with `.\Run-Sample.ps1` from the repository root. See [sample guide](../samples/CourtTimeline.WinUI.Sample/README.md) for the six scenarios and a distributable Windows build.
...
3. Turning plan off removes potential stages/planned events and clears hidden selection. Turning it back on does not invent a selection.
...
7. With “Только план”, hide the plan and verify that selection clears even without an active stage. With the dense and year-boundary scenarios, inspect lane separation and dates.
...
Cross-platform tests exercise the real calendar/layout code, including a leap day, inclusive stage ends, axis clipping, future active stages, hidden selections, dense events, empty input, invalid IDs and DateOnly limits. They do not verify native rendering or replace the Windows checks above.
